#!/usr/bin/env bash
# P003-S008: probe runner (VXLAN). Idêntico ao s007-probes.sh, só o dir de evidência.
...
# Uso: s008-probes.sh <case_id> <base_url> <path> <decision> [N] [ROUNDS]
set -u
CASE_ID="${1:?case_id}"
BASE="${2:?base_url}"
P="${3:?path}"
...
OUT="/opt/poc-k8s-fabric-studies/studies/p003/evidence/S008-2026-09-22T0210Z/probes"
mkdir -p "$OUT"
now() { date -u +%FT%TZ; }
