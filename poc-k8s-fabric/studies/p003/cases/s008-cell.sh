#!/usr/bin/env bash
# P003-S008: driver por celula (VXLAN) — placement + coleta discriminante + probes.
# Diferencas vs s007-cell.sh:
#   - espera de convergencia 60s apos pods estaveis (lição D-S007-4: sample
#     1s pos-rollout degrada a coleta da perna app)
#   - captura em cilium_vxlan (encapsulamento) + MTU medido
#   - filtro de underlay mais largo (IPs dos nos + IPs dos pods)
# Roda no host vm-cilium (root).
# Uso: s008-cell.sh <cell_id> <ingress_node> <auth_node> <app_node> [N] [ROUNDS]
set -u
CELL="${1:?cell_id}"
ING="${2:?ingress_node}"
AUTHN="${3:?auth_node}"
...
# convergencia xDS/dataplane (lição D-S007-4): espera fixa antes da coleta
log "converge_wait ${CONVERGE_WAIT}s"
sleep "$CONVERGE_WAIT"
...
# ------------------------------------------------------------- 3. routes + MTU
{
  ...
  echo "# MTU medido"
  for n in "$NODEA" "$NODEB"; do
    echo -n "  $n eth0: "; sudo docker exec "$n" cat /sys/class/net/eth0/mtu 2>&1
    echo -n "  $n cilium_vxlan: "; sudo docker exec "$n" cat /sys/class/net/cilium_vxlan/mtu 2>&1 || echo "n/a"
    echo -n "  $n cilium_wg0: "; sudo docker exec "$n" cat /sys/class/net/cilium_wg0/mtu 2>&1 || echo "n/a"
  done
} > "$C/routes.txt"
log "routes+mtu recorded"
...
t0=$(date +%s)
# underlay: bridge docker (perna inter-nos 172.19.0.x; VXLAN udp/8472 ou WG udp/51871)
FILTER="host 172.19.0.6 or host 172.19.0.7"
[ -n "$AUTH_IP_POD" ] && FILTER="$FILTER or host $AUTH_IP_POD"
[ -n "$APP_IP_POD" ] && FILTER="$FILTER or host $APP_IP_POD"
timeout "$CAPDUR" tcpdump -i "$BRIDGE" -w "$C/underlay-sample.pcap" \
  "$FILTER" >"$C/tcpdump-underlay.log" 2>&1 &
TDPID=$!
# cilium_vxlan (encapsulamento) e cilium_wg0 (W1/W2) nos nos
for n in "$NODEA" "$NODEB"; do
  if sudo docker exec "$n" ip link show cilium_vxlan >/dev/null 2>&1; then
    sudo docker exec -d "$n" timeout "$CAPDUR" tcpdump -i cilium_vxlan -w /tmp/vxlan-$n.pcap
    log "vxlan capture started on $n"
  fi
...
bash "$HERE/s008-probes.sh" "${CELL}-SAMP" "http://$ING_IP:$NP" /protected-http allow 10 1 >>"$C/cell.log" 2>&1
bash "$HERE/s008-probes.sh" "${CELL}-SAMP" "http://$ING_IP:$NP" /protected-http deny 10 1 >>"$C/cell.log" 2>&1
...
for n in "$NODEA" "$NODEB"; do
  if sudo docker exec "$n" test -f "/tmp/vxlan-$n.pcap" 2>/dev/null; then
    sudo docker cp "$n:/tmp/vxlan-$n.pcap" "$C/vxlan-$n.pcap" >/dev/null 2>&1
    sudo docker exec "$n" rm -f "/tmp/vxlan-$n.pcap"
  fi
...
# ------------------------------------------------------------- 5. probes completos
bash "$HERE/s008-probes.sh" "${CELL}-allow" "http://$ING_IP:$NP" /protected-http allow "$N" "$ROUNDS" >>"$C/cell.log" 2>&1
bash "$HERE/s008-probes.sh" "${CELL}-deny"  "http://$ING_IP:$NP" /protected-http deny  "$N" "$ROUNDS" >>"$C/cell.log" 2>&1
...
if [ -f "$C/underlay-sample.pcap" ]; then
  ...
  VXP8472=$(tcpdump -r "$C/underlay-sample.pcap" "udp port 8472" 2>/dev/null | wc -l)
  UPKTS=$(tcpdump -r "$C/underlay-sample.pcap" 2>/dev/null | wc -l)
else
  CLEARTEXT=n/a; WGPKTS=n/a; VXP8472=n/a; UPKTS=n/a
fi
{
  ...
  echo "underlay: packets=$UPKTS vxlan8472=$VXP8472 udp51871=$WGPKTS cleartext_sample_ids=$CLEARTEXT"
  if [ -f "$C/vxlan-$NODEA.pcap" ]; then echo "vxlan-A packets=$(tcpdump -r "$C/vxlan-$NODEA.pcap" 2>/dev/null | wc -l)"; fi
  if [ -f "$C/vxlan-$NODEB.pcap" ]; then echo "vxlan-B packets=$(tcpdump -r "$C/vxlan-$NODEB.pcap" 2>/dev/null | wc -l)"; fi
