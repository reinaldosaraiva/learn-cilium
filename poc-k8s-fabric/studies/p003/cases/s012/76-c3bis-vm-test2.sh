#!/usr/bin/env bash
# P003-S012 C3-bis — agentes frescos + VM TESTE (cgroupns=host).
set -uo pipefail

echo "== [1] agentes (fresco) =="
...
sudo docker exec p003-os bash -c 'ip netns list 2>/dev/null | sed "s/^/  ns: /"; ovs-vsctl list-ports br-int | sed "s/^/  port: /"'
...
echo "  --- cgroup machine/:"
sudo docker exec p003-os bash -c 'ls /sys/fs/cgroup/machine/ 2>/dev/null | grep -viE "^cgroup|^cpu" | head -3 | sed "s/^/    /"'
